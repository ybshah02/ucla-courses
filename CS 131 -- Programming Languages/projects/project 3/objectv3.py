...
class ObjectDef:
    ...
    def call_method(self, method_name, actual_params, super_only, line_num_of_caller):
        ...
        elif status == ObjectDef.STATUS_THROW and return_value is not None:
    ...
    def __execute_statement(self, env, return_type, code):
        ...
        if tok == InterpreterBase.BEGIN_DEF:
            ...
            return self.__execute_return(env, Type(InterpreterBase.STRING_DEF), code)
        else:
    ...
    def __execute_try(self, env, return_type, code):
        try_status, return_value = self.__execute_statement(env, return_type, code[1])
        if try_status == ObjectDef.STATUS_THROW:
            env.block_nest()
            self.__add_locals_to_env(env, [[InterpreterBase.STRING_DEF, InterpreterBase.EXCEPTION_VARIABLE_DEF, f'""']] ,code[0].line_num)
            env.set(InterpreterBase.EXCEPTION_VARIABLE_DEF, return_value.value())
            catch_status, return_value = self.__execute_statement(env, return_type, code[2])
            env.block_unnest()
            if catch_status == ObjectDef.STATUS_THROW:
                    return catch_status, return_value
    ...
    def __add_locals_to_env(self, env, var_defs, line_number):
        for var_def in var_defs:
            ...
            if '@' in var_def[0]:
               self.interpreter.is_template(var_def[0], line_number)
            elif var_def[0] in self.interpreter.class_index and self.interpreter.class_index[var_def[0]].is_template():
                if '@' not in var_def[0]:
            ...
            #print(var_type.type_name, default_value.type().type_name)
    ...
    def __execute_return(self, env, return_type, code):
        ...
        if code[0] == InterpreterBase.THROW_DEF:
            return ObjectDef.STATUS_THROW, result
        else:
    ...
    def __execute_print(self, env, code):
        ...
        for expr in code[1:]:
            ...
            if status == ObjectDef.STATUS_THROW and expr != InterpreterBase.EXCEPTION_VARIABLE_DEF:
                return status, term
    ...
    def __set_variable_aux(self, env, var_name, value, line_num):
        ...
        if self.__set_local_or_param(env, var_name, value, line_num):  # may report a type error
            return
    ...
    def __evaluate_expression(self, env, expr, line_num_of_statement):
        if type(expr) is not list:
            ...
            if expr == InterpreterBase.EXCEPTION_VARIABLE_DEF:
                if var_def:
                    return ObjectDef.STATUS_THROW, var_def
                else:
                    self.interpreter.error(ErrorType.NAME_ERROR, "invalid field exception", line_num_of_statement)
        ...
        if operator in self.binary_op_list:
            status_op1, operand1 = self.__evaluate_expression(env, expr[1], line_num_of_statement)
            status_op2, operand2 = self.__evaluate_expression(env, expr[2], line_num_of_statement)
            if status_op1 == ObjectDef.STATUS_THROW:
                return status_op1, operand1
            if status_op2 == ObjectDef.STATUS_THROW:
                return status_op2, operand2
            if (
        ...
        if operator in self.unary_op_list:
            status, operand = self.__evaluate_expression(env, expr[1], line_num_of_statement)[1]
            if status == ObjectDef.STATUS_THROW:
    ...
    def __execute_call_aux(self, env, code, line_num_of_statement):
        ...
        if obj_name == InterpreterBase.ME_DEF:
            ...
            status, obj_val = self.__evaluate_expression(env, obj_name, line_num_of_statement)
            if status == ObjectDef.STATUS_THROW:
