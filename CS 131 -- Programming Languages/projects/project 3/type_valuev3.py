...
def create_default_value(type_def):
    ...
    elif type_def == Type(InterpreterBase.EXCEPTION_VARIABLE_DEF):
        return Value(Type(InterpreterBase.EXCEPTION_VARIABLE_DEF), "")
    elif type_def == Type(
...
class TypeManager:
    ...
    def check_type_compatibility(self, typea, typeb, for_assignment):
        if '@' in typea.type_name or '@' in typeb.type_name:
            typea_split = typea.type_name.split('@')
            typeb_split = typeb.type_name.split('@')
            for a,b in zip(typea_split, typeb_split):
                if (self.is_valid_type(a) and not self.is_valid_type(b)) or (a != b and b != 'null' and a != 'null'):
                    return False
            return True
        # if either type is invalid (E.g., the user referenced a class name that doesn't exist) then
        # return false
        if not self.is_valid_type(typea.type_name) or not self.is_valid_type(typeb.type_name):
            return False
