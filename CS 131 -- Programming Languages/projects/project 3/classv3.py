...
class ClassDef:
    ...
    def __create_field_list(self, class_body):
        ...
        for member in class_body:
            ...
            if member[0] == InterpreterBase.FIELD_DEF:
                ...
                if '@' in member[1]:
                    test_mem = member[1].split('@')[0]
                else:
                    test_mem = member[1]
                if (test_mem == self.name and self.template) or (test_mem in self.interpreter.class_index and self.interpreter.class_index[test_mem].is_template()):
                    if '@' not in member[1]:
                        self.interpreter.error(ErrorType.TYPE_ERROR)
                    split_name = member[1].split('@')
                    if test_mem in self.interpreter.class_index:
                        params = self.interpreter.class_index[test_mem].get_params()
                    else:
                        params = self.params
                    if len(params) != len(split_name[1:]):
                        self.interpreter.error(ErrorType.TYPE_ERROR)
    ...
    def __create_variable_def_from_field(self, field_def):
        if len(field_def) > 3:
            ...
            var_def = VariableDef(Type(field_def[1]), field_def[2], create_default_value(Type(field_def[1])))
    ...
    def __create_method_list(self, class_body):
        ...
        for member in class_body:
            if member[0] == InterpreterBase.METHOD_DEF:
                ...
                #self.__check_templating(class_body)
    ...
    def __check_templating(self, class_body):
        #print(class_body)
        for member in class_body:
            if isinstance(member, list):
                for elem in member:
                    if isinstance(elem, list):
                        self.__check_templating(elem)

                    if member[0] == InterpreterBase.TEMPLATE_CLASS_DEF:
                        print(elem)
