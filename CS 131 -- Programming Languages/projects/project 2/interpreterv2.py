...
class Interpreter(InterpreterBase):
    ...
    def run(self, program):
        ...
        if not status:
            super().error(ErrorType.SYNTAX_ERROR, f"Parse error on program: {parsed_program}")
    ...
    def __get_all_class_names(self, program):
        all_class_names = []
        for item in program:
            if item[0] == InterpreterBase.CLASS_DEF:
                ...
                all_class_names.append(item[1])
        
        return all_class_names

    def __map_class_names_to_class_defs(self, program):
        self.class_index = {}
        all_class_names = self.__get_all_class_names(program)
        for item in program:
            if item[0] == InterpreterBase.CLASS_DEF:
                if item[1] in self.class_index:
                    super().error(
                ...
                self.class_index[item[1]] = ClassDef(item, self, self.class_index, all_class_names)
