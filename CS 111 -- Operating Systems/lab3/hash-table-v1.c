...
struct hash_table_v1 {
	...
	pthread_mutex_t mutex_lock; // declare lock
};
...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int cont = pthread_mutex_init(&hash_table->mutex_lock, NULL); // initialize mutex
	if (cont!=0)
	{
		exit(cont);
	}
                             ...
                             uint32_t value)
{
	...
	int cont = pthread_mutex_lock(&hash_table->mutex_lock); // lock table to minimize collisions
	if (cont!=0)
	{
		exit(cont);
	}
	...
	if (list_entry != NULL) {
		...
		cont = pthread_mutex_unlock(&hash_table->mutex_lock); // unlock after data has been updated
		if (cont!=0)
		{
			exit(cont);
		}
	...
	cont = pthread_mutex_unlock(&hash_table->mutex_lock); // unlock after data has been added
	if (cont!=0)
	{
		exit(cont);
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	int cont = pthread_mutex_destroy(&hash_table->mutex_lock); // unlock after data has been updated
	if (cont!=0)
	{
		exit(cont);
	}
