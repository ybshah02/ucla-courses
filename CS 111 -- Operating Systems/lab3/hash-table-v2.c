...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int cont = pthread_mutex_init(&entry->mutex_lock, NULL);
		if (cont != 0)
		{
			exit(cont);
		}
                             ...
                             uint32_t value)
{
	...
	int cont = pthread_mutex_lock(&hash_table_entry->mutex_lock);
	if (cont != 0)
	{
		exit(cont);
	}
	...
	if (list_entry != NULL) {
		...
		cont = pthread_mutex_unlock(&hash_table_entry->mutex_lock);
		if (cont != 0)
		{
			exit(cont);
		}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int cont = pthread_mutex_destroy(&entry->mutex_lock);
		if (cont != 0)
		{
			exit(cont);
		}
