...
(defun at-least-one-color (n c k)
  (let (
    (arr (list (node2var n c k)))
  )
    (cond 
      ((<= c k) (append arr (at-least-one-color n (+ c 1) k)))
      (t ())
    )
  )
)
...
(defun at-most-one (n c m k)
  (let (
    (var_c (- (node2var n c k)))
    (var_m (- (node2var n (+ k 1) m)))
  )
  
    (cond 
      ((<=  k m) (append (list (list var_c var_m)) (at-most-one n c m (+ k 1))))
      (t ())
    )
  )
)
...
(defun at-most-one-color (n c k)
    (cond 
      ((= c k) ())
      (t (append (at-most-one n c k c) (at-most-one-color n (+ c 1) k)))
    )
)
...
(defun generate-edge (n m c k)

  (let (
    (var_n (- (node2var n c k)))
    (var_m (- (node2var m c k)))
  )
  
    (cond 
      ((<=  c k) (append (list (list var_n var_m)) (generate-edge n m (+ c 1) k)))
      (t ())
    )
  )
)
...
(defun generate-edge-clauses (e k)
    (generate-edge (car e) (cadr e) 1 k)
)
...
(graph-coloring-to-sat "graph2.txt" "cnf22-7.cnf" 7)
(graph-coloring-to-sat "graph2.txt" "cnf22-8.cnf" 8)