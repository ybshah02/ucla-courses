...
(defun node2var (n c k)
  (+ c (* k (- n 1))) ; calculates given function
)
...
(defun at-least-one-color (n c k)
  (cond ((> c k) nil)
        (t (append (list (node2var n c k)) (at-least-one-color n (+ c 1) k)) ; lists every possible varible assignment
  	)
  )
)
...
; helper function: for every c < k in at-most-one-color, produces a list of negatieve clauses
(defun help-at-most-one-color (n c k c1)
  (cond ((> c1 k) nil)
  	(t (append (list (list (- (node2var n c k)) (- (node2var n (+ c1 1) k)))) (help-at-most-one-color n c k (+ c1 1)))
  	)
  )
)
...
(defun at-most-one-color (n c k)
  (cond ((= c k) nil)
  	(t (append (help-at-most-one-color n c k c) (at-most-one-color n (+ c 1) k)) ; append together negative clauses
  	)
  )
)
...
(defun generate-node-clauses (n k)
  (cons (at-least-one-color n 1 k) (at-most-one-color n 1 k)) ; combines at least and at most one color to create list of clauses that means exactly one color
)
...
; for the gven edge, recursively creates a clause for each color that they cannot both be that color
(defun help-generate-edge-clauses (e c k)
  (cond ((> c k) ())
	(t (append (list (list (- (node2var (first e) c k)) (- (node2var (second e) c k)))) (help-generate-edge-clauses e (+ 1 c) k))
	)
  )
)

(defun generate-edge-clauses (e k)
  (help-generate-edge-clauses e 1 k) ; generate clauses using helper
)
...
(graph-coloring-to-sat "graph2.txt" "cnf2-7.cnf" 7)
(graph-coloring-to-sat "graph2.txt" "cnf2-8.cnf" 8)
